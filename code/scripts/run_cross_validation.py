...
# 11894,  6179,  5715
# 2060 2069 2059    1905 1905 1905 
# 3965 3965 3964 

X_fold1 = x_train[:3965]
X_fold2 = x_train[3965:7930]
X_fold3 = x_train[7930:]

Y_fold1 = y_train[:3965]
Y_fold2 = y_train[3965:7930]
Y_fold3 = y_train[7930:]
...
influences_on_7 = np.zeros((7930, 3))
influences_on_1 = np.zeros((7930, 3))
...
for X_train, Y_train, X_test, Y_test in zip([X_train1, X_train2, X_train3], [Y_train1, Y_train2, Y_train3], [X_fold3, X_fold2, X_fold3], [Y_fold3, Y_fold2, Y_fold3]):
    ...
    #pcaVis(class_wise_7, clusters_7, centroids_7, "for sevens")
    ...
    #pcaVis(class_wise_1, clusters_1, centroids_1, "for ones")
...
fold1_infl = np.concatenate((influences_on_7[:3965, :2], influences_on_1[:3965, :2]), axis=1)
print(influences_on_7.shape, influences_on_1.shape)
fold2_infl = np.concatenate((influences_on_7[3965:, 0][:,np.newaxis], influences_on_7[:3965, 2][:,np.newaxis], influences_on_1[3965:, 0][:,np.newaxis], influences_on_1[:3965, 2][:,np.newaxis]), axis=1)
...
fold3_infl = np.concatenate((influences_on_7[3965:7929, 1:], influences_on_1[3965:7929, 1:]), axis=1)
